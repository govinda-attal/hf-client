...
func ErrClientInitFailed() status.ErrServiceStatus {
	return status.ErrServiceStatus{
		status.ServiceStatus{Code: codes.ErrInternal, Message: "Failed to initialize fabric ca client"}, nil,
	}
}
...
func ErrOperationFailed() status.ErrServiceStatus {
	return status.ErrServiceStatus{
		status.ServiceStatus{Code: codes.ErrInternal, Message: "Failed to perform operation with fabric ca"}, nil,
	}
}
